...
Zen 模型根據端點使用四種不同的 AI SDK 轉接器：

| 轉接器 | 端點 | 模型 |
| ------ | ---- | ---- |
| `@ai-sdk/openai` | `/v1/responses` | 所有 GPT 5.x 模型 |
| `@ai-sdk/anthropic` | `/v1/messages` | 所有 Claude 模型 |
...
四種轉接器在協議層面均為 OpenAI API 相容 — 均遵循標準 REST 契約（JSON 請求/回應、SSE 流式傳輸等）。區別在於使用的 **AI SDK 套件**，這決定了各供應商請求/回應的處理方式（header、body 構造、認證等）。
